...
sourceStreamJSONFilePath = 'abfss://bronze@adlsudadalakehousedev.dfs.core.windows.net/daily-pricing-streaming-source-data'

@dlt.table()
def dlt_daily_pricing_matrialized_view():
    return (spark
            .read
            .format("json")
...
def dlt_daily_pricing_streaming_trans_2():
    return(dlt.read("dlt_daily_pricing_streaming_table"))
...
def dlt_daily_pricing_streaming_trans_3():
    return (dlt.read_stream("dlt_daily_pricing_streaming_table")
            .dropDuplicates(["ROW_ID","DATETIME_OF_PRICING"])  )


# COMMAND ----------

badRecordsrules = {}
badRecordsrules["incorrect_records"] = f"NOT({'AND'.join(dqRules.values())})"
@dlt.table
...
dlt.apply_changes(
    ...
    keys = ["ROW_ID", "DATETIME_OF_PRICING"],
